...
def paris_now():
    ...
    return now_utc + timedelta(hours=paris_offset_hours(now_utc))
...
def main():
    ...
    print(f'[{now_paris:%H:%M:%S}] Live refresh for {today}', flush=True)
    ...
    def run_fetcher(fetcher_name: str):
        ...
        try:
            ...
            evs = fn(today) or []
            return fetcher_name, evs, None, time.time() - started
        except Exception as e:
            return fetcher_name, [], e, time.time() - started

    events = []
    fetch_errors = []
    fetcher_names = ('fetch_soccer_day', 'fetch_basket_day', 'fetch_other_day', 'fetch_tennis_day')
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(fetcher_names)) as ex:
        futures = {ex.submit(run_fetcher, name): name for name in fetcher_names}
        for future in concurrent.futures.as_completed(futures):
            fetcher_name, evs, error, elapsed = future.result()
            if error:
                fetch_errors.append(f'{fetcher_name}: {type(error).__name__}: {str(error)[:80]}')
                print(f'  WARN {fetcher_name} failed after {elapsed:.1f}s: {error}', flush=True)
                continue
            events.extend(evs)
            print(f'  {fetcher_name}: {len(evs)} events in {elapsed:.1f}s', flush=True)

    skip_espn_odds = os.environ.get('FETCH_LIVE_SKIP_ESPN_ODDS') == '1'
    n_en = 0
    if skip_espn_odds:
        print('  ESPN odds enrichment skipped for fast desktop refresh', flush=True)
    else:
        try:
    ...
    print(f'[{paris_now():%H:%M:%S}] Done in {elapsed:.1f}s · data.js={DATA_JS.stat().st_size/1024:.0f}KB · status={status}', flush=True)
