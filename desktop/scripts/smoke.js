...
async function main() {
  ...
  try {
    ...
    const expectedHomeTop = Math.min(3, dashboard.homeTableReadyMarkets.filter(Boolean).length);
    if (dashboard.rows < 15 || dashboard.rows > 32 || dashboard.timeline < 8 || dashboard.trackButtons < minTrackButtons || dashboard.safeBadges < minSafeBadges || !/24h|aujourd’hui|à venir|surveill/i.test(dashboard.metricLabel) || (dashboard.metric < 6 && !/trop strict|Winamax|0 prêt/i.test(dashboard.funnelAlert)) || !dashboard.homeShell || dashboard.homeTop3 < expectedHomeTop || dashboard.homeTableRows < 6 || dashboard.homeTableRows > 8 || dashboard.homeSortButtons < 4 || !hasActionCopy || hasComplexMarket || hasTechnicalJargon || hasExpertRepairLabel || /Écouter le brief|brief audio|SpeechSynthesis|TTS/i.test(dashboard.dashboardText)) {
